...
class SoloMpcGaitGen:
    ...
    def create_cnt_plan(self, q, v, t, v_des, yaw_des):
    ...
    def optimize(self, q, v, t, v_des, yaw_des, step_height, current_contact, X_wm = None, F_wm = None, P_wm = None):

        #TODO: Move to C++
        t1 = time.time()
        self.step_height = step_height
        self.current_contact = current_contact
        self.create_cnt_plan(q, v, t, v_des, yaw_des)

        #Creates costs for IK and Dynamics
        self.create_costs(q, v, v_des, yaw_des, t)
        ...
        #For balancing, try to track 0 momentum
        mom_opt[0:int(round(self.ik_horizon/self.gait_dt))] = 0.0
        mom_opt[int(round(self.ik_horizon/self.gait_dt))] = 0.0

        #Track CoM between two point contacts (for now, hard-coded and calculated from initial position of URDF)
        com_opt[0:int(round(self.ik_horizon/self.gait_dt))] = [0.2, 0.0, 0.26]
        com_opt[int(round(self.ik_horizon/self.gait_dt))] = [0.2, 0.0, 0.26]
