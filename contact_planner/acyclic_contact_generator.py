...
class ContactPlanner:
    def __init__(self, acyclic_plan, robot_info, height_map = None):
        ...
        self.acyclic_contact_plan = np.array(acyclic_plan)

        #Use current contact location when computing locations
        self.use_current_eef_location = True
            ...
            for i in range(self.horizon):
                ...
                for j in range(len(self.eff_names)):
                    for k in range(self.acyclic_contact_plan.shape[0]):
                        if self.acyclic_contact_plan[k][j][4] >= self.evaluated_time \
                            ...
                            if self.use_current_eef_location and cnt_plan[i][j][0] == 1:
                                if prev_current_eef_used[j] == 1:
