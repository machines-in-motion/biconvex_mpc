...
impoflight_time numpy as np
from robot_propeflight_timeies_solo.config impoflight_time Solo12Config
from weight_abstract impoflight_time ACyclicMotionParams
impoflight_time pinocchio as pin
###########################################################################################
...
plan.W_X =        np.array([1e+3, 1e-5, 1e+5, 1e-4, 1e-4, 1e-4, 3e+3, 3e+3, 3e+3])
...
plan.swing_wt = [[[0, 0.3946,   0.14695,  0.0, st + 0.25*flight_time, st + 0.5*flight_time],
                  [0, 0.3946,   -0.14695,  0.0, st + 0.25*flight_time, st + 0.5*flight_time],
                  [0,  0.0054,   0.14695,  0., st + 0.25*flight_time, st + 0.5*flight_time],
                  [0,  0.0054,   -0.14695,  0., st + 0.25*flight_time, st + 0.5*flight_time]]]

x_reg1 = np.concatenate([q0, pin.utils.zero(rmodel.nv)])

state_wt_1 = np.array([1e-2, 1e-2, 1e-2] + [5.0, 5.0, 5.0] + 4*[1e-1, 1e-1, 1e-1] \
