...
namespace motion_planner{
    ...
    KinoDynMP::KinoDynMP(std::string urdf, double m, int n_eff, int dyn_col, int ik_col):
        ...
        x0.resize(rmodel_.nq + rmodel_.nv); x0.setZero();
        X_wm.resize(9*(dyn_col+1)); X_wm.setZero();
        F_wm.resize(3*n_eff*dyn_col); F_wm.setZero();
        P_wm.resize(9*(dyn_col+1)); P_wm.setZero();
        dyn_com_opt.resize(dyn_col+1, 3); dyn_com_opt.setZero();
        dyn_mom_opt.resize(dyn_col+1, 6); dyn_mom_opt.setZero();

        ik_com_opt.resize(ik_col+1, 3); ik_com_opt.setZero();
        ik_mom_opt.resize(ik_col+1, 6); ik_mom_opt.setZero();
